...
public class CommentDAO {
	...
	public List<CommentBean> getComment_BoardList(int cNum){
		
		List<CommentBean> boardList = new ArrayList<CommentBean>();
		
		try{
			con = getConnection();
			sql = "select * from comment_board where cNum=? order by num desc";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, cNum);
	...
	public int delete_CommentBoard(String id, int num){
		...
		try {
			con = getConnection();
			sql = "delete from comment_board where num=? and id=?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, num);
			pstmt.setString(2, id);
	...
	public int update_CommentBoard(String id, int num, String comment){
		int check = 0;
			
		try {
			con = getConnection();
			sql = "update comment_board set comment=? where num=? and id=?";
			pstmt = con.prepareStatement(sql);
			...
			System.out.println("update_CommentBoard메소드 내부에서 예외 발생 : " + e);
		} finally {
			resourceClose();
		}
		return check;
