	...
	%>
		...
		<div class="board-search">
			...
			<div class="board_search_button">
				...
				<input type="button" value="글쓰기" onclick="location.href='write.jsp?pageNum=<%=pageNum%>&search=<%=search%>&opt=<%=opt%>'"/>
			</div>
		</div>
...
<%
			if(pageBlock > 0){
				if(pageBlock_count != 1){
					if(currentPage%10 == 0){
%>
						<a href="board.jsp?pageNum=<%= ((currentPage-11)/10*10)+1 %>&search=<%=search%>&opt=<%=opt%>">이전</a>
<%
					}
					else{
%>
						<a href="board.jsp?pageNum=<%= ((currentPage-10)/10*10)+1 %>&search=<%=search%>&opt=<%=opt%>">이전</a>
<%						
					}
				} 
			}
...
<%
			...
			if(pageBlock > 0){
			   if( pageBlock_count != pageBlock){
				   if(currentPage%10 == 0){
%>
						<a href="board.jsp?pageNum=<%= currentPage+1 %>&search=<%=search%>&opt=<%=opt%>">다음</a>
<%					   
				   }
				   else{
%>
						<a href="board.jsp?pageNum=<%= ((currentPage+10)/10*10)+1 %>&search=<%=search%>&opt=<%=opt%>">다음</a>
<%
				   }
			   } 
