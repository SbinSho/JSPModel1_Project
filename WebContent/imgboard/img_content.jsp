...
<%
	...
	serverFileName = bBean.getServerFileName();
...
%>
						<a href="../data/data_download.jsp?path=upload&name=<%=serverFileName%>"><%=serverFileName%></a>
<%						
					}
%>						
				</td>
			...
			<tr>
				<td colspan="4"><img class="img_content1" src="<%= request.getContextPath()%>/upload/<%=serverFileName%>"></td>			
			</tr>
			<tr>
				<th colspan="4">사진 설명</th>
			</tr>
			<tr>
				<td colspan="4">
...
%>	
				<input type="button" 
					   value="글수정" 
					   class="btn" 
					   onclick="location.href='img_updateCheck.jsp?num=<%=DBnum%>&pageNum=<%=pageNum%>&DBID=<%=DBId%>&search=<%=search%>&opt=<%=opt%>'">
					   
				<input type="button" 
					   value="글삭제" 
					   class="btn" 
					   onclick="location.href='img_delete.jsp?num=<%=DBnum%>&pageNum=<%=pageNum%>&DBID=<%=DBId%>&search=<%=search%>&opt=<%=opt%>'">
...
<script>
	  ...
	  function comment() {
		    ...
		    $.ajax({
		        type:'post',
// 		        url : "http://180.68.97.42:8080/JspProject/imgboard/img_commentPro.jsp",
		        url : "img_commentPro.jsp", 
	  ...
	  function updatePro(checkNum) {
			...
			$.ajax({
				type:'POST',
// 		        url : "http://180.68.97.42:8080/JspProject/imgboard/img_comment_UpdatePro.jsp",
		        url : "img_comment_UpdatePro.jsp", 
		        dataType : "text",
	  ...
	  function getCommentList(){
		  ...
		  var num = $("#DBnum").val();
		    $.ajax({
		        type:'GET',
// 		        url : "http://180.68.97.42:8080/JspProject/imgboard/img_comment_ListPro.jsp",
		        url : "img_comment_ListPro.jsp", 
		        dataType : "json",
		        data:{DBnum : num},
	...
	function deleteCheck(checkNum) {
			...
			if(confirm("정말 삭제하시겠습니까??")){
				$.ajax({
			        type:'GET',
// 			        url : "http://180.68.97.42:8080/JspProject/imgboard/img_comment_deletePro.jsp",
			        url : "img_comment_deletePro.jsp",
			        dataType : "text",
			        data:{num : num},
			        success : function(data){
			        	if(data == 1){
